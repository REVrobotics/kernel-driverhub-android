...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    if (bgt911_lx){
        send_cfg_buf[0] = gtp_dat_gt911_lx;
        cfg_info_len[0] =  CFG_GROUP_LEN(gtp_dat_gt911_lx);
    }
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (of_property_read_u32(np, "revert-x-y", &revert)) {
        dev_err(&client->dev, "no revert-x-y defined\n");
        revert = 0;
    }
    if (revert == 1) {
        gtp_x_reverse = !gtp_x_reverse;
    } else if (revert == 2) {
        gtp_y_reverse = !gtp_y_reverse;
    } else if (revert == 3) {
        gtp_x_reverse = !gtp_x_reverse;
        gtp_y_reverse = !gtp_y_reverse;
    }
