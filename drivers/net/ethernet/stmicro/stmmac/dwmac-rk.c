...
static int rk_gmac_probe(struct platform_device *pdev)
{
	...
	if(of_property_read_bool(pdev->dev.of_node, "vendor,fix_rtl8201f")){
        dev_info(&pdev->dev, "Vendor fix rtl8201f led0\n");
        ndev = (struct net_device *)dev_get_drvdata(&pdev->dev);
        priv = netdev_priv(ndev);
        if(priv->mii){
            value = (uint32_t)priv->mii->read(priv->mii, priv->plat->phy_addr, 19);
            priv->mii->write(priv->mii, priv->plat->phy_addr, 31, 7);
            value &= ~(1u << 10u);
            priv->mii->write(priv->mii, priv->plat->phy_addr, 19, value);
            priv->mii->write(priv->mii, priv->plat->phy_addr, 31, 0);
        }
    }
